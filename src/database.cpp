...
void ItemDatabase::initialize(const std::string& db_name) {
  if (db) {
    sqlite3_close(db);
    db = nullptr;
  }
...
void ItemDatabase::openDatabase(const std::string& db_name) {
  if (sqlite3_open(db_name.c_str(), &db)) {
    RCLCPP_ERROR(rclcpp::get_logger("database"),
...
bool ItemDatabase::insertItem(const Item& item) {
  ...
  if (itemExists(item)) {
    RCLCPP_WARN(rclcpp::get_logger("database"),
  ...
  const char* sql = "INSERT INTO Items (id, timestamp, type, name, text, url, feature, extra) "
...
bool ItemDatabase::itemExists(const Item& item) {
  return urlExists(item.url);
}

bool ItemDatabase::urlExists(const std::string& url) {
  const char* sql = "SELECT COUNT(*) FROM Items WHERE url = ?;";
  sqlite3_stmt* stmt;
  int result = 0;
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    RCLCPP_ERROR(rclcpp::get_logger("database"),
...
bool ItemDatabase::isValidItem(const Item& item) {
  return !item.feature.empty();
}

std::string ItemDatabase::join(const std::vector<std::string>& vec, const std::string& delimiter) {
...
std::vector<std::string> ItemDatabase::split(const std::string& s, char delimiter) {
...
bool ItemDatabase::deleteItem(int id) {
...
Item ItemDatabase::queryItem(int id) {
  const char* sql = "SELECT timestamp, type, name, text, url, feature, extra FROM Items WHERE id = ?;";
  sqlite3_stmt* stmt;
  Item item;
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
    RCLCPP_ERROR(rclcpp::get_logger("database"),
...
bool ItemDatabase::executeSQL(const char* sql) {
    char* errMsg = nullptr;
...
std::vector<Item> ItemDatabase::queryItemsByPage(int page_number, int page_size) {
    std::shared_lock<std::shared_mutex> lock(mutex_); // 读锁

    const char* sql = "SELECT id, timestamp, type, name, text, url, feature, extra FROM Items LIMIT ? OFFSET ?;";
    sqlite3_stmt* stmt;
