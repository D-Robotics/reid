...
void AiMsgManage::Feed(
  ...
  RCLCPP_INFO(
      rclcpp::get_logger("reid_msg_manage"), "%s", ss.str().c_str());

  reid_feed_cache_.Feed(msg);
}
...
int AiMsgManage::GetTargetRois(
  ...
  ai_msg = reid_feed_cache_.Get(msg_ts, time_out_ms);
  if (!ai_msg) {
    RCLCPP_WARN(rclcpp::get_logger("reid_msg_manage"),
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("reid_msg_manage"),
               "Frame ai ts: %s targets size: %d",
               ts.c_str(),
               ai_msg->targets.size());
  size_t body_roi_idx = 0;
  for (const auto target : ai_msg->targets) {
    ...
    for (const auto& roi : target.rois) {
      RCLCPP_DEBUG(rclcpp::get_logger("reid_msg_manage"),
                   "roi.type: %s",
                   roi.type.c_str());
      if ("person" == roi.type || "body" == roi.type) {
        RCLCPP_DEBUG(rclcpp::get_logger("reid_msg_manage"),
        ...
        if (0 == norm_func(&raw_roi, &normed_roi)) {
          ...
          valid_roi_idx[body_roi_idx] = rois->size() - 1;

          RCLCPP_DEBUG(rclcpp::get_logger("reid_msg_manage"),
                       "Valid person roi map: %d %d",
                       body_roi_idx,
                       valid_roi_idx[body_roi_idx]);
        }
